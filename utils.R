...
csc_to_h5 <- function(mat, outfile, group = NULL, createnewfile = T, native = F,
                      save_dimmnames = T) {
  #' Convert Matrix or Sparse Matrix to HDF5 File
  #'
  #' This function converts a dense or sparse matrix object to an HDF5 file.
  #' It supports both dense and supported sparse matrix formats (dgRMatrix,
  #' dgCMatrix, dgTMatrix). The matrix is stored in the Compressed Sparse
  #' Column (CSC) format in the HDF5 file. Optionally, it can also save
  #' dimension names if present.
  #'
  #' @param mat A matrix or a sparse matrix object (e.g., dgCMatrix).
  #' @param outfile Path to the output HDF5 file.
  #' @param group Optional character name for the group in the outfile HDF5 file.
  #' @param createnewfile Logical indicating whether to create a new HDF5 file
  #' (default is TRUE) or to write toan existing h5 file.
  #' @param native Logical. If TRUE, array-like objects
  ...
  #' @param save_dimnames Logical indicating whether to save dimension names if
  #' present (default is TRUE).
  #'
  #' @return None. The function writes the matrix data to the specified HDF5 file.
  #'
  #' @examples
  #' # Example 1: Convert a dense matrix to HDF5
  #' data <- matrix(1:9, nrow = 3)
  #' csc_to_h5(data, "dense_matrix.h5")
  #'
  #' # Example 2: Convert a sparse matrix to HDF5
  #' library(Matrix)
  #' sp_matrix <- Matrix(c(0, 0, 0, 4, 5, 0, 7, 0, 0), nrow = 3, sparse = TRUE)
  #' csc_to_h5(sp_matrix, "sparse_matrix.h5")
  #'
  #' # Example 3: Convert a matrix with dimnames to HDF5
  #' data <- matrix(1:6, nrow = 2, dimnames = list(c("A", "B"), c("X", "Y", "Z")))
  #' csc_to_h5(data, "matrix_with_dimnames.h5")
  #'
  #' # Example 4: Convert a matrix with NULL dimnames to HDF5
  #' data <- matrix(1:6, nrow = 2, dimnames = list(NULL, c("X", "Y", "Z")))
  #' csc_to_h5(data, "matrix_with_null_dimnames.h5")
  #'
  #' @importFrom rhdf5 h5write h5checktypeOrOpenLoc H5Lexists h5closeAll
  #' @importFrom rhdf5 h5createFile
  #' @importFrom methods is.matrix is.null
  #' @importFrom Matrix is(dgRMatrix, dgCMatrix, dgTMatrix)
  ...
  if (!inherits(mat, c("Matrix", "dgRMatrix", "dgCMatrix", "dgTMatrix", "matrix")))
    stop("mat must be a dense or sparse matrix.")

  # Ensure mat is a dgCMatrix
  if (!inherits(mat, c("dgCMatrix"))) {
    mat <- as(mat, "dgCMatrix")
  }

  # Get triplet and sparse matrix attributes
  mat_dim <- mat@Dim
  triplet <- as.list(summary(mat))
  attr(triplet, "encoding_type") <- "csc_matrix"
  attr(triplet, "dims") <- mat_dim

  # Add dimnames if requested
  if (save_dimmnames) {
    mat_dimnames <- mat@Dimnames
    names(mat_dimnames) <- paste0("dimnames_", 1:length(mat_dimnames))
    mat_dimnames <- mat_dimnames[!unlist(lapply(mat_dimnames, is.null))]
    triplet <- c(triplet, mat_dimnames)
  }

  # Set group name to object name
  if (is.null(group)) {
    call_args <- match.call(expand.dots = FALSE)
    group <- as.character(call_args$mat)
  }

  # Open or create h5 file
  if (createnewfile && file.exists(outfile)) file.remove(outfile)
  loc <- rhdf5:::h5checktypeOrOpenLoc(outfile, createnewfile = createnewfile,
                                      native = native)

  # Save triplet and attributes if group doesn't exist
  if (rhdf5::H5Lexists(loc$H5Identifier, group)) {
    message("Can not create group. Object with name '", group, "' already exists.")
  } else {
    rhdf5::h5write(triplet, loc$H5Identifier, group, write.attributes = T)
  }
