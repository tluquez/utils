...
def minimize_scalar(fun, bounds, args=(), method='hybrid-brent', tol=None,
                    options=dict(maxiter=np.iinfo('int32').max), n_trials=100,
                    n_jobs=1):
    ...
    study.optimize(lambda trial: fun(
        trial.suggest_float('x', *bounds), *args), n_trials=n_trials,
        n_jobs=n_jobs)
...
def best_leiden(nn_graph, underlying_data, batch_labels=None, metric='ray_turi',
                method='hybrid-brent', bounds=(0, 1), n_trials=100, n_jobs=1,
                verbose=True):
